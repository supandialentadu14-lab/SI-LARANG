...
@section('subheader', 'Isi data untuk membuat berita acara')
...
@section('content')
    <script>
        window.formData = function () {
            return {
                ...
                prefill() { this.items = {!! json_encode(($data['items'] ?? [])) !!}; },
    ...
    <form method="POST" action="{{ route('reports.pinjam.report') }}" x-data="formData()" x-init="$nextTick(() => updatePembuka())" class="bg-white rounded-lg shadow p-6 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
           <div>
            ...
            <div>
                ...
                <input x-ref="tanggal" @change="updatePemboka()" type="date" name="tanggal" value="{{ $data['tanggal'] ?? now()->toDateString() }}" class="w-full px-4 py-2 rounded-lg border border-gray-300" required>
            </div>
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">Tempat</label>
                <input x-ref="tempat" @input="updatePemboka()" type="text" name="tempat" value="{{ $data['tempat'] ?? ($opd->nama_opd ?? '') }}" class="w-full px-4 py-2 rounded-lg border border-gray-300" required>
            </div>
        </div>
